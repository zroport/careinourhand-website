  ...
  try {
    ...
    for (const faq of faqs) {
      ...
      if (existing.rows.length > 0) {
        await client.query(
          `UPDATE "Faq" SET answer = $1, category = $2, "order" = $3, "isActive" = $4, "updatedAt" = $5 WHERE question = $6`,
          [faq.answer, faq.category, faq.order, true, now, faq.question]
        );
        updated++;
      } else {
        const id = cuid();
        await client.query(
          ...
          [id, faq.question, faq.answer, faq.category, faq.order, true, now, now]
        );
        inserted++;
      }
    }

    console.log(`✅ FAQs seeded: ${inserted} inserted, ${updated} updated`);
  } catch (err) {
